...
<script type="text/javascript">
	function doPrintNormalCost(){
		$('#normalCostPrintArea').printArea();
	}
</script>
</head>
<body>
<div id="normalCostPrintArea" align="center" >
<div style="height:150px"></div>
<table  style="font-size:12px;margin-left:130px;">
	<tr style="height:44px;">
	  <td width="260" ></td>
	  <td width="260" align="center" style="letter-spacing:10px;font-size:18px;font-weight:bold;">${costs.lzfs }费用报销单</td>
	  <td width="260" align="center" style="font-size:8px;">单证编号：第${costs.dzbh }号</td>
	</tr>
	<tr>
	  <td width="260" align="left">部门：${costs.deptName }</td>
...
<table class="printTable" style="font-size:12px;margin-left:130px;">
	...
	<tbody>
		<c:forEach var="row" items="${costs.rows }" varStatus="i">
			<tr>
				<td>${row.cshiyou }</td>
				<td>${row.subjectId}${row.subjectName}</td>
				<td>${row.zrzxName}</td>
				<td><fmt:formatNumber value="${row.camount}" pattern="#0.00" /></td>
				<c:if test="${i.first}">
				<td rowspan="8" ></td>
				</c:if>
			</tr>
		</c:forEach>
		<c:if test ="${costs.total <8}" >
			<c:forEach begin="${costs.total}" end="7" varStatus="i">
				<tr>
					<td></td>
					<td></td>
					<td></td>
		...
		<tr>
			<td>合计：    ${costs.lzfs }</td>
			<td>￥：<fmt:formatNumber value="${costs.amount }" pattern="#0.00" /></td>
			<td colspan="3">人民币：${costs.upperNumber }</td>
		</tr>
	</tbody>
</table>
<table style="font-size:12px;margin-left:130px;">
...
<br />
<div align="center">
<input type="button" value="打印单据" onclick="doPrintNormalCost()" />
