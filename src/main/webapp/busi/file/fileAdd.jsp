...
{  
    ...
    (  
        {  
            ...
            fileElementId:'file0',//文件上传空间的id属性  <</span>input type="file" id="file" name="file" />  
            dataType: 'text',//返回值类型 一般设置为json  
            success: function (d,status)  //服务器成功响应处理函数  
            {  
            	var json=$.parseJSON(d);
               if (json.state){
            	   $('#file_add_path').val(json.url);
               }
	...
	<form method="post">
		...
		<table  class="input_table">
			...
			<tr>
				...
				<td><input type="text" name="cfilepath" id="file_add_path" class="easyui-validatebox" readonly="readonly" data-options="required:'true',missingMessage:'请上传文件！'" style="width:250px"/></td>
			</tr>
		</table>
	</form>
	<table class="input_table">
		<tr>
				...
				<td >
					<div class="file-box">
						<input type='text' name='textfield' id='textfield0' class='txt1' />
						 <input type='button' class='btn1' value='浏览...' /> 
						 <input type="file" name="file" class="file1" id="file0" size="28" onchange="document.getElementById('textfield0').value=this.value" /> 
						 <input type="button" name="submit" class="btn1" value="上传" onclick="return file_ajaxFileUpload();"/>
					</div>
				</td>
			</tr>
	</table>
